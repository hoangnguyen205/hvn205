...
void loop() 
{
  ...
  //int ppm_pwm = random(800, 1000);
  //int rco2=random(700, 1000);
  /**
  ...
  device_manager->data_core_co2.CO2=ppm_pwm;


  if(millis() > last_update_screen + 15000)
  {
    last_update_screen = millis();
    user_interface.display.updateData(device_manager->getDataCore().temperature, device_manager->getDataCore().humidity, device_manager->getDataCore().CO2, device_manager->getDataCore().pm2_5, device_manager->getMAC());
    user_interface.display.updateClock(device_manager->getTime().hour, device_manager->getTime().minute);
  ...
  ThingSpeak.setField(1,device_manager->data_core_co2.CO2);
  ThingSpeak.setField(2, device_manager->data_core_co2.temperature);
  ThingSpeak.setField(3, device_manager->data_core_co2.humidity);
  ThingSpeak.setField(4, device_manager->data_core_co2.pm2_5);
